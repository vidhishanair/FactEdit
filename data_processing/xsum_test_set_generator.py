...
#model = BartForConditionalGeneration.from_pretrained("facebook/bart-large-xsum", force_bos_token_to_be_generated=True).to('cuda')
...
fcc_test_file = os.path.join('output_dir/bart_large_xsum/predict3/', "data-dev.jsonl")
dae_test_file = os.path.join('output_dir/bart_large_xsum/predict3/', "dae_input.txt")

json_wp = open("data/bart_xsum_test_R145_sent_wspan_wid.json", "w")
...
with open(fcc_test_file, "w") as fccp, open(dae_test_file, "w") as daep:
    for idx, data in enumerate(dataset["test"]):
        ...
        summary_ids = model.generate(inputs['input_ids'], num_beams=4, max_length=140, min_length=55, early_stopping=True)
        ...
        daep.write(str(source_text).replace("\n", " ")+"\n"+str(generated_summary[0]).replace("\n", " ")+"\n\n")
...
res_fp = open('output_dir/bart_large_xsum/predict3/results.txt', 'w')
