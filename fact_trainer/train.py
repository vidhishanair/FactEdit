...
def safely_get_last_checkpoint(training_args):
...
def main():
    ...
    last_checkpoint = safely_get_last_checkpoint(training_args)
    ...
    def preprocess_function(examples):
        source_article = examples["source_article"]
        orig_summ = examples["original_summary"]
        gen_summ = examples["generated_summary"]
        labels = examples["label"]

        # inputs = [prefix + inp for inp in inputs]
        inputs = [gen + " <sep> " + art for gen, art in zip(gen_summ, source_article)]
        targets = orig_summ
    ...
    if training_args.do_predict:
        ...
        orig_test_dataset = copy.deepcopy(test_dataset)
    ...
    if training_args.do_predict:
        ...
        if trainer.is_world_process_zero():
            if training_args.predict_with_generate:
                ...
                fcc_test_file = os.path.join(training_args.output_dir, "data-dev.jsonl")

                with open(output_test_preds_file, "w") as writer, open(fcc_test_file, "w") as fccp:
                    for idx, pred in enumerate(test_preds):
                        source_article = orig_test_dataset[idx]["source_article"]
                        summary_doc = spacy_nlp(pred)
                        for summary_sent in summary_doc.sents:
